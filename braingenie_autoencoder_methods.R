...
retrain_autoencoder_gtex = function(gene_list = NULL, output = "", tissue = NULL, ncomps = 20, n_folds = 5){
  ...
  for(fold in 1:max(folds)){
    ...
    model %>%
      layer_dense(units = 500, input_shape = ncol(x_train)) %>%
...
recover_latent_space = function(use_n_comps = 20, newdata = NULL){
